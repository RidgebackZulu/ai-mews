...
// --- DST-safe-ish scheduling ---
// GitHub Actions cron is UTC; we trigger at both 13:00 and 14:00 UTC.
// Only proceed if it's 08:00 in America/Chicago.
...
function scoreResult(r) {
  ...
  // Bias toward “popular highlights” / broadly shareable.
  const boosts = [
...
// De-dupe by hostname and URL to keep variety.
...
for (const r of raw) {
  ...
  // allow max 2 per host
  const hostCount = [...seenHost].filter(h => h === host).length;
  if (hostCount >= 2) continue;

  seenUrl.add(r.url);
  seenHost.add(host);
  candidates.push(r);
  if (candidates.length >= 8) break; // fetch a few extras in case readability fails
}
...
const siteBase = process.env.SITE_BASEURL || ''; // not required; best-effort
const pageUrl = siteBase ? `${siteBase}/posts/${today}/` : `https://ridgebackzulu.github.io/ai-mews/posts/${today}/`;
